...
import { Scan, Send, Download, Bell } from 'lucide-react';
...
export default function HomeScreen() {
  ...
  const quickActions = [
    { icon: Scan, label: 'Scan', color: 'hsl(45, 100%, 51%)' },
    { icon: Send, label: 'Pay', color: 'hsl(217, 91%, 60%)' },
    { icon: Download, label: 'Receive', color: 'hsl(142, 71%, 45%)' }
  ];
  ...
  return (
    <div className="min-h-screen bg-background w-full max-w-md mx-auto pb-24">
      {/* Header */}
      <div className="px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <Avatar initial="RK" color="hsl(45, 100%, 51%)" size="lg" />
          <div>
            <Text variant="h3" weight="bold" className="text-foreground block">
              Rajesh Kumar
            </Text>
      ...
      <div className="px-5 py-4">
        <BalanceCard
          ...
          validThru="07/29"
          status="ACTIVE"
          gradient="linear-gradient(135deg, hsl(217, 91%, 60%), hsl(173, 80%, 40%))"
        />
      </div>
