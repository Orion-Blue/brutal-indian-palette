...
export const registry: ComponentMeta[] = [
  ...
  },
  {
    ...
    variants: [
      { name: 'Title Only', render: () => <Header title="Profile" /> },
      { name: 'Title + Subtitle', render: () => <Header title="Send Money" subtitle="Find a contact" /> },
      { name: 'With Back Button', render: () => <Header title="Send Money" showBack /> },
      { name: 'With Action', render: () => <Header title="Profile" rightAction={<button className="rounded-xl px-4 h-10 bg-card border border-border/30 text-sm font-semibold">UPI ID</button>} /> },
    ],
    sizes: ['full-width mobile, 56px min height'],
    states: ['default','focus'],
    props: [
      { name: 'title', type: 'string' },
      { name: 'subtitle', type: 'string' },
      { name: 'rightAction', type: 'React.ReactNode' },
      { name: 'showBack', type: 'boolean', default: 'false' },
      { name: 'onBackClick', type: '() => void' },
    ],
    tokens: { spacing: ['space-md', 'space-lg'], typography: ['type-h2','type-body-sm','type-caption'], colors: ['foreground', 'muted-foreground', 'card', 'border'] },
    accessibility: ['Semantic header element. Back button (if present) has aria-label="Go back". Title is h1 (or h2 if subtitle).'],
    usageNotes: ['Page header at top of each screen. Back button (44px min) on left for navigation screens. Right action button (44px min). Min 56px height for touch.'],
